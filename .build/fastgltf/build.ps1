...
if (-not $SkipDownload) {
    Write-BuildLog "INF" $LogCtx "Fetching latest fastgltf release..."
    $release = Get-LatestRelease -Repo "spnda/fastgltf"
    ...
    $zipPath = "$cacheDir\fastgltf-source.zip"
    $extractPath = "$cacheDir\fastgltf-extract"
    ...
    foreach ($filePath in $filePaths) {
        $sourceFile = "$sourceRoot\$filePath"
        ...
        if (Test-Path $sourceFile) {
            ...
            Write-BuildLog "WRN" $LogCtx "File not found in source archive: $filePath"
        }
    }

    $licenseDir = Clear-Licenses -Name "fastgltf"
    $licenseFiles = @("LICENSE.md", "README.md")
...
$includeDir = Clear-Includes -Name "fastgltf"
Copy-Item "$ScriptDir\include\fastgltf\*" "$includeDir\" -Force
...
Copy-Libs -SourceDir $buildDir -LibName "fastgltf.lib" -RepoRoot $RepoRoot
...
Write-BuildLog "INF" $LogCtx "Done! fastgltf $tag build complete."
